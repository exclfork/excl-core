...
CzEXCLTracker::~CzEXCLTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzEXCLTracker::Init()
{
...
bool CzEXCLTracker::Archive(CMintMeta& meta)
{
...
bool CzEXCLTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzEXCLTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzEXCLTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzEXCLTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzEXCLTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzEXCLTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
CAmount CzEXCLTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzEXCLTracker::GetUnconfirmedBalance() const
{
...
bool CzEXCLTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzEXCLTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzEXCLTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzEXCLTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzEXCLTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzEXCLTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzEXCLTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzEXCLTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzEXCLWallet* zEXCLWallet)
{
    bool iszEXCLWalletInitialized = (NULL != zEXCLWallet);
    CMintMeta meta;
    ...
    if (! iszEXCLWalletInitialized)
        zEXCLWallet = new CzEXCLWallet(strWalletFile);
    meta.isSeedCorrect = zEXCLWallet->CheckSeed(dMint);
    if (! iszEXCLWalletInitialized)
        delete zEXCLWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzEXCLTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzEXCLTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzEXCLTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzEXCLTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzEXCLTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzEXCLTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    ...
    if (fUpdateStatus) {
        ...
        delete zEXCLWallet;
        LogPrint("zero", "%s: added %d dzexcl from DB\n", __func__, listDeterministicDB.size());
    }
