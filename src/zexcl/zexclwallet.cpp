...
CzEXCLWallet::CzEXCLWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zexcl seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZEXCLSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZEXCLSeed_deprecated()) {
                    LogPrintf("%s: Updated zEXCL seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zexcl seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zexcl wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzEXCLWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZEXCLCount(nCountLastUsed);
    else if (!walletdb.ReadZEXCLCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzEXCLWallet::Lock()
{
    seedMaster = 0;
}

void CzEXCLWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzEXCLWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZEXCL(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzEXCLWallet::LoadMintPoolFromDB()
{
...
void CzEXCLWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzEXCLWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzEXCLWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zexclTracker->HasPubcoinHash(pMint.first)) {
...
bool CzEXCLWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZEXCL(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zexclTracker which also adds to database
    pwalletMain->zexclTracker->Add(dMint, true);
...
void CzEXCLWallet::SeedToZEXCL(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzEXCLWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzEXCLWallet::GenerateDeterministicZEXCL(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzEXCLWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzEXCLWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzEXCLWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
