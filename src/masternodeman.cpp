...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    LogPrintf("void CMasternodeMan::ProcessMessage\n");
        CMasternodeBroadcast mnb;
        vRecv >> mnb;

        if (mapSeenMasternodeBroadcast.count(mnb.GetHash())) { //seen
            masternodeSync.AddedMasternodeList(mnb.GetHash());
            LogPrintf("void CMasternodeMan::ProcessMessage seen\n");
            return;
        }
        ...
        if (mnb.CheckInputsAndAdd(nDoS)) {
            // use this as a peer
            LogPrintf("void CMasternodeMan::ProcessMessage add \n");
            addrman.Add(CAddress(mnb.addr), pfrom->addr, 2 * 60 * 60);
            masternodeSync.AddedMasternodeList(mnb.GetHash());
        } else {
            LogPrintf("masternode mnb - Rejected Masternode entry %s\n", mnb.vin.prevout.hash.ToString());
    ...
    // Light version for OLD MASSTERNODES - fake pings, no self-activation
        ...
        if (fAcceptable) {
            ...
            if (mi != mapBlockIndex.end() && (*mi).second) {
                CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 5000 EXCL tx -> 1 confirmation
