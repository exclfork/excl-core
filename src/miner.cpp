...
//    const int nHeight = pindexPrev->nHeight + 1;
    ...
    if (nHeight <= 110) {
        ...
        CScript A_2_SCRIPT = GetScriptForDestination(CBitcoinAddress("EfK1BM23tbrChnFuKwFfhqXbX8HiAM4uWU").Get());
        CScript A_3_SCRIPT = GetScriptForDestination(CBitcoinAddress("EMNhMH9g3951EcbWEmzq3PZmLNBjwU1tXP").Get());
        CScript A_4_SCRIPT = GetScriptForDestination(CBitcoinAddress("EdqzSYjWLYVxFtpBQEyYppZFrgftkRJ2vx").Get());
        CScript A_5_SCRIPT = GetScriptForDestination(CBitcoinAddress("ESiAT3myvD3eewpAE5ifuY3vWfSeLSNdw3").Get());
        CScript A_6_SCRIPT = GetScriptForDestination(CBitcoinAddress("EZhWXiTRE3Us8Vzdm9B5fvcrxe9TaXsmYL").Get());
        CScript A_7_SCRIPT = GetScriptForDestination(CBitcoinAddress("ELsBrf4Jtqrs9YhYL3QdaENYB1om29f3fe").Get());
        CScript A_8_SCRIPT = GetScriptForDestination(CBitcoinAddress("EYtqUJPz6nDD4SdHW4g2ntGCdyRxFcmmYj").Get());
        CScript A_9_SCRIPT = GetScriptForDestination(CBitcoinAddress("EdbBbWFw4AKacFeVZMvHJn95WycZ2C2rW5").Get());
        CScript A_10_SCRIPT = GetScriptForDestination(CBitcoinAddress("EM6AiG1wbUvwLZ5SzEXJeHkbpayUu2Pmi6").Get());
        // And give it to the founders
        txNew.vout.push_back(CTxOut(reward, CScript(A_1_SCRIPT.begin(), A_1_SCRIPT.end())));
        txNew.vout.push_back(CTxOut(reward, CScript(A_2_SCRIPT.begin(), A_2_SCRIPT.end())));
        txNew.vout.push_back(CTxOut(reward, CScript(A_3_SCRIPT.begin(), A_3_SCRIPT.end())));
        txNew.vout.push_back(CTxOut(reward, CScript(A_4_SCRIPT.begin(), A_4_SCRIPT.end())));
        txNew.vout.push_back(CTxOut(reward, CScript(A_5_SCRIPT.begin(), A_5_SCRIPT.end())));
        txNew.vout.push_back(CTxOut(reward, CScript(A_6_SCRIPT.begin(), A_6_SCRIPT.end())));
        txNew.vout.push_back(CTxOut(reward, CScript(A_7_SCRIPT.begin(), A_7_SCRIPT.end())));
        txNew.vout.push_back(CTxOut(reward, CScript(A_8_SCRIPT.begin(), A_8_SCRIPT.end())));
        txNew.vout.push_back(CTxOut(reward, CScript(A_9_SCRIPT.begin(), A_9_SCRIPT.end())));
        txNew.vout.push_back(CTxOut(reward, CScript(A_10_SCRIPT.begin(), A_10_SCRIPT.end())));
    }
    ...
    CAmount nFees = 0;
    {
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                // zEXCL spends can have very large priority, use non-overflowing safe functions
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("EXCLMiner : generated block is stale");
    }
...
{
    LogPrintf("EXCLMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("excl-miner");
...
//                    LogPrintf("%s: failed to find zEXCL with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
//                    continue;
//                }
//
//                //Sign block with the zEXCL key
//                if (!SignBlockWithKey(*pblock, key)) {
//                    LogPrintf("BitcoinMiner(): Signing new block with zEXCL key failed \n");
//                    continue;
//                }
...
//            }
        ...
        LogPrintf("Running EXCLMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
