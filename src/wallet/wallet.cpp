...
/**
 * Fees smaller than this (in uexcl) are considered zero fee (for transaction creation)
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zEXCL in the future, for now it's useless,
        ...
        CAmount nBalance = 0;
        std::vector<CMintMeta> vMints = zexclTracker->GetMints(true);
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        std::set<CMintMeta> setMints = zexclTracker->ListMints(true, true, true);
...
bool CWallet::MintableCoins()
{
    ...
    CAmount nZexclBalance = GetZerocoinBalance(false);
    ...
    while (nLockAttempts < 100) {
        ...
        for (auto &it : mapMintsSelected) {
            ...
            if (!GetTransaction(mint.GetTxHash(), txMint, hashBlock)) {
                receipt.SetStatus(strprintf(_("Unable to find transaction containing mint %s"), mint.GetTxHash().GetHex()), ZEXCL_TXMINT_GENERAL);
                return false;
            } else if (mapBlockIndex.count(hashBlock) < 1) {
                // check that this mint made it into the blockchain
                receipt.SetStatus(_("Mint did not make it into blockchain"), ZEXCL_TXMINT_GENERAL);
                return false;
            }
            ...
            if (outputIndex == -1) {
                receipt.SetStatus(_("Pubcoin not found in mint tx"), ZEXCL_TXMINT_GENERAL);
                return false;
            }

            mint.SetOutputIndex(outputIndex);
            CTxIn in;
            if(!ZEXCLModule::createInput(in, mint, hashTxOut)){
                receipt.SetStatus(_("Cannot create public spend input"), ZEXCL_TXMINT_GENERAL);
                return false;
            }
            vin.emplace_back(in);
...
std::string CWallet::ResetMintZerocoin()
{
    ...
    std::set<CMintMeta> setMints = zexclTracker->ListMints(false, false, true);
...
std::string CWallet::GetUniqueWalletBackupName(bool fzexclAuto) const
{
...
void CWallet::ZExclBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (boost::filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                boost::filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozexclbackup-%d.dat", i + 1);
    ...
    if(!GetArg("-zexclbackuppath", "").empty()) {
        boost::filesystem::path customPath(GetArg("-zexclbackuppath", ""));
