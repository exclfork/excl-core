...
public:
    ...
    bool WriteZEXCLSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZEXCLSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZEXCLSeed_deprecated(uint256& seed);
    bool EraseZEXCLSeed();
    bool EraseZEXCLSeed_deprecated();

    bool WriteZEXCLCount(const uint32_t& nCount);
    bool ReadZEXCLCount(uint32_t& nCount);
