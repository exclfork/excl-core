...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zexclspends need to use the cs_spendcache
...
static const CAmount MASTERNODE_COIN_REQUIRED = 5000 * COIN;
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT5000IFMN = 3,
    ONLY_NONDENOMINATED_NOT5000IFMN = 4, // ONLY_NONDENOMINATED and not 5000 EXCL at the same time
    ONLY_5000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zEXCL send
enum ZerocoinSpendStatus {
    ZEXCL_SPEND_OKAY = 0,                            // No error
    ZEXCL_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZEXCL_WALLET_LOCKED = 2,                         // Wallet was locked
    ZEXCL_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZEXCL_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZEXCL_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZEXCL_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZEXCL_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZEXCL_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZEXCL_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZEXCL_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZEXCL_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZEXCL_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZEXCL_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZEXCL_SPENT_USED_ZEXCL = 14,                      // Coin has already been spend
    ZEXCL_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZEXCL_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZExclBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZEXCLOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzexclAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzEXCLTracker> zexclTracker;
    ...
    void setZWallet(CzEXCLWallet* zwallet)
    {
        zwalletMain = zwallet;
        zexclTracker = std::unique_ptr<CzEXCLTracker>(new CzEXCLTracker(strWalletFile));
    }

    CzEXCLWallet* getZWallet() { return zwalletMain; }
...
//        return fEnableZeromint || fEnableAutoConvert;
    ...
    void setZExclAutoBackups(bool fEnabled)
    {
    ...
    /** zEXCL reset */
    boost::signals2::signal<void()> NotifyzEXCLReset;
