...
/** Fees smaller than this (in uexcl) are considered zero fee (for relaying and mining)
...
bool CheckZerocoinSpend(const CTransaction& tx, bool fVerifySignature, CValidationState& state, bool fFakeSerialAttack)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (isPublicSpend) {
            ...
            if (!ZEXCLModule::parseCoinSpend(txin, tx, prevOut, publicSpend)){
        ...
        if (isPublicSpend) {
            ...
            if (!ZEXCLModule::validateInput(txin, prevOut, tx, ret)){
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZEXCLModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
                ...
                for (const CTxIn &txin : tx.vin) {
                    ...
                    if (txin.scriptSig.IsZerocoinSpend() || isPublicSpend) {
                        ...
                        if (isPublicSpend) {
                            ...
                            PublicCoinSpend publicSpend(params);
                            CValidationState state;
                            if (!ZEXCLModule::ParseZerocoinPublicSpend(txin, tx, state, publicSpend)) {
    ...
    if (isPoS) {
        ...
        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.scriptSig.IsZerocoinSpend()){
                ...
                exclInputs.push_back(stakeIn);
            }
        }
        const bool hasEXCLInputs = !exclInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasEXCLInputs)
                    ...
                    for (const CTxIn& exclIn : exclInputs){
                        if(exclIn.prevout == in.prevout){
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction &t : bl.vtx) {
                    for (const CTxIn &in: t.vin) {
                        ...
                        for (const CTxIn &stakeIn : exclInputs) {
