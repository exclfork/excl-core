...
#include "zexcl/accumulatormap.h"
#include "addrman.h"
#include "alert.h"
...
#if defined(NDEBUG)
#error "EXCL cannot be compiled without assertions."
#endif
...
//        return error("%s : zEXCL spend with serial %s is already in block %d\n", __func__,
...
//{
//    //Check to see if the zEXCL is properly signed
//    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
//        try {
//            if (!spend.HasValidSignature())
//                return error("%s: V2 zEXCL spend does not have a valid signature\n", __func__);
...
//            return error("%s: trying to spend zEXCL without the correct spend type. txid=%s\n", __func__,
//                         tx.GetHash().GetHex());
//        }
//    }
//
...
//        if (!isBlockBetweenFakeSerialAttackRange(pindex->nHeight))
//            return error("%s : zEXCL spend with serial %s from tx %s is not in valid range\n", __func__,
...
//    if (!CheckTransaction(tx, false, true, state, isBlockBetweenFakeSerialAttackRange(chainHeight)))
...
//                return state.Invalid(error("AcceptToMemoryPool : zEXCL spend tx %s already in block %d",
...
//                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zexcl");
//            }
//        } else {
            LOCK(pool.cs);
            ...
            // Check that zEXCL mints are not already known
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZEXCLStake)
{
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from EXCL
...
{
    RenameThread("excl-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZEXCLMinted()
{
    CBlockIndex *pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating minted ZEXCL..."), 0);
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted ZEXCL..."), percent);
        }
...
void RecalculateZEXCLSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating spent ZEXCL..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()))));
            uiInterface.ShowProgress(_("Recalculating spent ZEXCL..."), percent);
        }

        //Rewrite zEXCL supply
        CBlock block;
        assert(ReadBlockFromDisk(block, pindex));
        ...
        //Remove spends from zEXCL supply
...
bool RecalculateEXCLSupply(int nHeightStart)
{
...
//        nSupplyPrev = CAmount(5449796547496199);
    ...
    uiInterface.ShowProgress(_("Recalculating EXCL supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating EXCL supply..."), percent);
        }
...
bool UpdateZEXCLSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
    ...
    // A one-time event where only the zEXCL supply was off (due to serial duplication off-chain on main net)
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s ZExclSpent: %s\n",
...
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert EXCL to zEXCL
...
//    bool fZerocoinActive = block.GetBlockTime() > Params().Zerocoin_StartTime();
    ...
    for (const CTransaction& tx : block.vtx) {
        if (!CheckTransaction(
                tx,
                false, //fZerocoinActive,
                blockHeight >= Params().Zerocoin_Block_EnforceSerialRange(),
                state,
                false//isBlockBetweenFakeSerialAttackRange(blockHeight)
        ))
            return error("CheckBlock() : CheckTransaction failed");

        // double check that there are no double spent zEXCL spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zEXCL serial %s in block\n Block: %s",
...
//        return error("%s: zEXCL stake block is less than allowed start height", __func__);
//
//    if (CZExclStake* zEXCL = dynamic_cast<CZExclStake*>(stake)) {
//        CBlockIndex* pindexFrom = zEXCL->GetIndexFrom();
//        if (!pindexFrom)
//            return error("%s: failed to get index associated with zEXCL stake checksum", __func__);
//
//        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
//            return error("%s: zEXCL stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
//
...
//        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zEXCL->GetValue()));
//        if (nChecksum200 != zEXCL->GetChecksum())
//            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zEXCL->GetChecksum(), nChecksum200);
//    } else {
...
//        if (stake->IsZEXCL() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
//            return state.DoS(100, error("%s: staked zEXCL fails context checks", __func__));
    ...
    if (isPoS) {
        ...
        std::vector<CTxIn> zEXCLInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.scriptSig.IsZerocoinSpend()){
                zEXCLInputs.push_back(stakeIn);
            }else{
                pivInputs.push_back(stakeIn);
            }
        }
        const bool hasEXCLInputs = !pivInputs.empty();
        const bool hasZEXCLInputs = !zEXCLInputs.empty();
        ...
        if (isBlockFromFork) {
            ...
            // Now that this loop if completed. Check if we have zEXCL inputs.
            if(hasZEXCLInputs){
                for (const CTxIn& ZExclInput : zEXCLInputs) {
                    CoinSpend spend = TxInToZerocoinSpend(ZExclInput);
...
//                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zexcl");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zEXCL mints and %d zEXCL spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // EXCL: We use certain sporks during IBD, so check to see if they are
        ...
        if(nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("zexcl", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("zexcl", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
