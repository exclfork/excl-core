...
#include "zexcl/accumulatorcheckpoints.h"
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "excl.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "excld.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
//    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the EXCL and zEXCL money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 23230, 51474));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in EXCL/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in EXCL/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, excl, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero, precompute, staking)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in EXCL/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all EXCL specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-exclstake=<n>", strprintf(_("Enable or disable staking functionality for EXCL inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zexclstake=<n>", strprintf(_("Enable or disable staking functionality for zEXCL inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:23230"));
...
//    strUsage += HelpMessageOpt("-backupzexcl=<n>", strprintf(_("Enable automatic wallet backups triggered after each zEXCL minting (0-1, default: %u)"), 1));
//    strUsage += HelpMessageOpt("-precompute=<n>", strprintf(_("Enable precomputation of zEXCL spends and stakes (0-1, default %u)"), 1));
...
//    strUsage += HelpMessageOpt("-zexclbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zEXCL backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizeexclamount=<n>     " + strprintf(_("Keep N EXCL anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The EXCL Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that EXCL is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. EXCL Core is shutting down."));
...
#endif
    // Make sure only a single EXCL process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. EXCL Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //EXCL: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //EXCL specific: zerocoin and spork DB's
                ...
                // EXCL: load previous sessions sporks if we have them.
...
//                        RecalculateZEXCLMinted();
//                        RecalculateZEXCLSpent();
//                    }
//                    // Recalculate from the zerocoin activation or from scratch.
//                    RecalculateEXCLSupply(reindexZerocoin ? Params().Zerocoin_StartHeight() : 1);
//                }

                if (GetBoolArg("-reindexmoneysupply", false)) {
                    RecalculateEXCLSupply(1);
                }
...
//                        }
//                        // EXCL: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of EXCL Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart EXCL Core to complete") << "\n";
        ...
        zwalletMain = new CzEXCLWallet(pwalletMain->strWalletFile);
        ...
        fVerifyingBlocks = false;
        // Disable zEXCLWallet
        //Inititalize zEXCLWallet
//        uiInterface.InitMessage(_("Syncing zEXCL wallet..."));

//        pwalletMain->InitAutoConvertAddresses();

//        bool fEnableZExclBackups = GetBoolArg("-backupzexcl", true);
//        bool fEnableZExclBackups = false;
//        pwalletMain->setZExclAutoBackups(fEnableZExclBackups);

        //Load zerocoin mint hashes to memory
//        pwalletMain->zexclTracker->Init();
...
//    }
//
//    nAnonymizeExclAmount = GetArg("-anonymizeexclamount", 0);
//    if (nAnonymizeExclAmount > 999999) nAnonymizeExclAmount = 999999;
//    if (nAnonymizeExclAmount < 2) nAnonymizeExclAmount = 2;
    ...
    LogPrintf("Anonymize EXCL Amount %d\n", nAnonymizeExclAmount);
    ...
    /* Denominations
       ...
       For example:
       1EXCL+1000 == (.1EXCL+100)*10
       10EXCL+10000 == (1EXCL+1000)*10
    */
...
//            // Run a thread to precompute any zEXCL spends
