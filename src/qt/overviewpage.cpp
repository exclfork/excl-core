...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::EXCL)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sEXCLPercentage, QString& szEXCLPercentage)
{
    int nPrecision = 2;
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount exclAvailableBalance = balance - immatureBalance - nLockedBalance;
    CAmount nUnlockedBalance = nTotalBalance - nLockedBalance;

    // EXCL Watch-Only Balance
...
//    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;
    ...
    // Combined balances
    CAmount availableTotalBalance = exclAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // EXCL labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, exclAvailableBalance, false, BitcoinUnits::separatorAlways));
...
//    }
    ...
    // EXCL Available
    bool showEXCLAvailable = settingShowAllBalances || exclAvailableBalance != nTotalBalance;
    bool showWatchOnlyEXCLAvailable = showEXCLAvailable || nAvailableWatchBalance != nTotalWatchBalance;
    ui->labelBalanceText->setVisible(showEXCLAvailable || showWatchOnlyEXCLAvailable);
    ui->labelBalance->setVisible(showEXCLAvailable || showWatchOnlyEXCLAvailable);
    ui->labelWatchAvailable->setVisible(showWatchOnlyEXCLAvailable && showWatchOnly);

    // EXCL Pending
    bool showEXCLPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyEXCLPending = showEXCLPending || watchUnconfBalance != 0;
    ui->labelPendingText->setVisible(showEXCLPending || showWatchOnlyEXCLPending);
    ui->labelUnconfirmed->setVisible(showEXCLPending || showWatchOnlyEXCLPending);
    ui->labelWatchPending->setVisible(showWatchOnlyEXCLPending && showWatchOnly);

    // EXCL Immature
    bool showEXCLImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = showEXCLImmature || watchImmatureBalance != 0;
    ui->labelImmatureText->setVisible(showEXCLImmature || showWatchOnlyImmature);
    ui->labelImmature->setVisible(showEXCLImmature || showWatchOnlyImmature); // for symmetry reasons also show immature label when the watch-only one is shown
    ui->labelWatchImmature->setVisible(showWatchOnlyImmature && showWatchOnly); // show watch-only immature balance

    // EXCL Locked
    bool showEXCLLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyEXCLLocked = showEXCLLocked || nWatchOnlyLockedBalance != 0;
    ui->labelLockedBalanceText->setVisible(showEXCLLocked || showWatchOnlyEXCLLocked);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("EXCL")
    updateDisplayUnit();
