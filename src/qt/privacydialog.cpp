                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zEXCL ought to be enough for anybody." - Bill Gates, 2017
    ui->zEXCLpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzEXCLSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zEXCLpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzEXCL_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zEXCL is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zEXCL, true));
        if (!ctx.isValid()) {
...
void PrivacyDialog::on_pushButtonSpendzEXCL_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zEXCL is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zEXCL, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zEXCL
        sendzEXCL();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zEXCL
    sendzEXCL();
}

void PrivacyDialog::on_pushButtonZExclControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZExclControlDialog* ZExclControl = new ZExclControlDialog(this);
    ZExclControl->setModel(walletModel);
    ZExclControl->exec();
}

void PrivacyDialog::setZExclControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelZExclSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzEXCL()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Excl Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zEXCLpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zEXCL is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " EXCL </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zEXCL</b>";
    ...
    // use mints from zEXCL selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZExclControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZExclControlDialog::GetSelectedMints();
...
//        fSuccess = pwalletMain->SpendZerocoin(nAmount, wtxNew, receipt, vMintsSelected, fMintChange, fMinimizeChange, &address);
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zEXCL was spent successfully update the addressbook with the label
    ...
    // Clear zexcl selector in case it was used
    ZExclControlDialog::setSelectedMints.clear();
    ui->labelZExclSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zEXCL Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Excl, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zEXCL Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zexclTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zEXCL </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zEXCL "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zEXCL "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zEXCL "));
    ui->labelzEXCLAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zEXCL </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zEXCL </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zEXCL </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzEXCLSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzEXCL->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zEXCL
        ui->pushButtonMintzEXCL->setEnabled(false);
        ui->pushButtonMintzEXCL->setToolTip(tr("zEXCL is currently disabled due to maintenance."));

        // Spend zEXCL
        ui->pushButtonSpendzEXCL->setEnabled(false);
        ui->pushButtonSpendzEXCL->setToolTip(tr("zEXCL is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zEXCL
        ui->pushButtonMintzEXCL->setEnabled(true);
        ui->pushButtonMintzEXCL->setToolTip(tr("PrivacyDialog", "Enter an amount of EXCL to convert to zEXCL", 0));

        // Spend zEXCL
        ui->pushButtonSpendzEXCL->setEnabled(true);
        ui->pushButtonSpendzEXCL->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
