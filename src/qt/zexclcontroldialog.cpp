...
#include "ui_zexclcontroldialog.h"
...
std::set<std::string> ZExclControlDialog::setSelectedMints;
std::set<CMintMeta> ZExclControlDialog::setMints;

bool CZExclControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZExclControlDialog::COLUMN_DENOMINATION || column == ZExclControlDialog::COLUMN_VERSION || column == ZExclControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZExclControlDialog::ZExclControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZExclControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZExclControlDialog::~ZExclControlDialog()
{
    delete ui;
}

void ZExclControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZExclControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZExclControlWidgetItem* itemDenom(new CZExclControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZExclControlWidgetItem *itemMint = new CZExclControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zexclTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zexclTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zEXCL.";
            else if (!mint.isSeedCorrect)
                strReason = "The zEXCL seed used to mint this zEXCL is not the same as currently hold in the wallet";
            else
...
void ZExclControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZExclControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
...
void ZExclControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
