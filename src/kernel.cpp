...
{
    if (!pindex)
        return error("GetLastStakeModifier: Null pindexNexnull pindex");
...
//    LogPrintf("tx.IsZerocoinSpend() %s", tx.GetHash().ToString().c_str());
//        libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txin);
//        if (spend.getSpendType() != libzerocoin::SpendType::STAKE)
//            return error("%s: spend is using the wrong SpendType (%d)", __func__, (int)spend.getSpendType());
//
//        stake = std::unique_ptr<CStakeInput>(new CZExclStake(spend));
//        return true;
//    } else
    {
        ...
        CExclStake* pivInput = new CExclStake();
